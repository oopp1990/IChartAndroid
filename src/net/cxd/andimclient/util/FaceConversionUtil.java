...
public class FaceConversionUtil {
	...
	public SpannableString getExpressionString(Context context, String str) {
		width = PositionUtil.getWidth((BaseActivity)context);	
	...
	private void dealExpression(Context context,
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				if (width > 1000) {
					bitwidth = width /4 ;
					Log.w("bitwidth", bitwidth + " ..");
				}else{
					bitwidth = width /6 ;
					Log.w("bitwidth", bitwidth + " ..");
				}
				bitmap = Bitmap.createScaledBitmap(bitmap, bitwidth, bitwidth, true);
				//bitmap.set
	...
	private void ParseData(List<String> data, Context context) {
		...
		try {
			for (String str : data) {
				...
				//Log.w("emoji", fileName + "   " + resID  + "    "+ context.getPackageName());
				if (resID != 0) {

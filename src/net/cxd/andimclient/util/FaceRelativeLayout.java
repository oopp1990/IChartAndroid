...
public class FaceRelativeLayout extends RelativeLayout implements
	...
	public void onClick(View v) {
		//Log.w("监听器", findViewById(R.id.btn_face) + " 查找控件");
		//Log.w("监听器",imm + " 查找控件" + findViewById(R.id.et_sendmessage).getWindowToken());
		switch (v.getId()) {
		case R.id.btn_face:
			// 隐藏表情选择框
			if (view.getVisibility() == View.VISIBLE) {
				view.setVisibility(View.GONE);
				//((ImageButton)findViewById(R.id.btn_face)).setImageResource(R.drawable.board_system);
				//imm.showSoftInput(findViewById(R.id.et_sendmessage), InputMethodManager.SHOW_FORCED);
				imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
			} else {
				view.setVisibility(View.VISIBLE);
				//((ImageButton)findViewById(R.id.btn_face)).setImageResource(R.drawable.board_system);
				imm.hideSoftInputFromWindow(findViewById(R.id.et_sendmessage).getWindowToken(), 0);
			}
			break;
		case R.id.et_sendmessage:
			...
			if (view.getVisibility() == View.VISIBLE) {
				view.setVisibility(View.GONE);
			//	findViewById(R.id.btn_face).setBackgroundResource(R.drawable.board_system);
				imm.showSoftInput(findViewById(R.id.et_sendmessage), 0);
			}
			break;
	...
	private void Init_viewPager() {
		...
		for (int i = 0; i < emojis.size(); i++) {
			...
			view.setPadding(0, 0, 0, 0);// last is 5
	...
	private void Init_Point() {
		...
		for (int i = 0; i < pageViews.size(); i++) {
			...
			layoutParams.width = 8; //last is 8
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		if (emoji.getId() == R.drawable.face_del_icon) {
			...
			if (selection > 0) {
				...
				//TODO
				//Log.w("删除"," 长度 " + text.length() );
				if (selection >= 5) {
					String str = text.substring(selection-5);
					//Log.w("删除"," 表情 " + str );
					String zhengze = "([/s]+:[1][4][0-1]|[/s]+:[1][0-3][0-9]|[/s]+:[0][0-9][1-9]|[/s]+:[0][1-9][0])";
